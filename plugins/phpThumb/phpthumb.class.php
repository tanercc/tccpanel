...
class phpthumb {
	...
	var $aoe  = 1;     // Allow Output Enlargement
	var $zaman = null;
	...
	var $config_error_image_height                   = 400;
		...
		if (is_null($versionstring)) {
			...
			if ($commandline) {
				...
				//if (preg_match('#^Version: [^0-9]*([ 0-9\\.\\:Q/\\-]+) (http|file)\:#i', $versionstring[1], $matches)) {
				  if (preg_match('/Version: ImageMagick ([0-9]*\.[0-9]*\.[0-9]*)/', $versionstring[1], $matches)) {
...
//echo $maxwidth.'x'.$maxheight.'<hr>';
	...
	function SetCacheFilename() {
		...
		$FilenameParameters1 = array('ar', 'bg', 'bc', 'far', 'sx', 'sy', 'sw', 'sh', 'zc', 'zaman');
